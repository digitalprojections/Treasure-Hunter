...
export function ExpeditionStory({state,island,viewport,hidden}:{state:GameState;island:number;viewport:RefObject<HTMLElement|null>;hidden:boolean}){
 ...
 return <>
 <ReadingControl><button className="story-launcher" aria-label="Open story conversation" aria-expanded={expanded} onClick={()=>setExpanded(open=>!open)}>Story</button></ReadingControl>
 {!hidden&&expanded&&<div ref={bubble} className="chapter-bubble" style={position} aria-label="Map conversation">
 <><div className="chapter-bubble-top"><span>{beat.speaker}</span><button aria-label="Close conversation" onClick={()=>setExpanded(false)}>×</button></div>
