...
import {tileRenderCounts, particleRenderCounts} from '../src/utils/renderDiagnostics';
...
const rewardEffect = { revision: 1, kind: 'treasure' as const };
...
function Check(){
 ...
 const run=async()=>{try{
   ...
   const particleRenders=particleRenderCounts.get('treasure') ?? 0;
   if(particleRenders!==1)throw Error('Particles rendered with hero clock: '+particleRenders);
   ...
   setReport(JSON.stringify({result:'PASS',tiles:144,particleRendersDuringWalk:particleRenders,staticRendersDuringIdle:idleOthers.length,walkRenderedTiles:walkCounts,revealRenderedTiles:revealChanged},null,2));
 ...
 <div style={{display:'grid',gridTemplateColumns:'repeat(12,32px)',width:384}}>{state.tiles.map(tile=>{const current=state.playerPos.x===tile.x&&state.playerPos.y===tile.y;return <TileComponent key={tile.id} tile={tile} revealEffect={current && animation==='walk'?rewardEffect:undefined} terrain={terrain.get(tile.id)} isCurrent={current} idleElapsedMs={0} playerAnimation={current?animation:'idle'} playerFacing="right" spriteClockMs={0} combat={null} onClick={noop} onRevealComplete={noop}/>;})}</div></div>;
}
